...
outline 只要 1 个 turn，就能把整部法规的"骨架"拿到，再选 `--in "<那部
...
chinalaw search "二倍工资 按月" --in "最高人民法院关于审理劳动争议案件适用法律问题的解释（二）" --kind article --format json
# → article_hits 第一条直接命中第 6 条
```

**reasoning 上的关键纪律**：fetch 完一定要 outline 或使用 `search --in` 做法规内检索，不要
...
某些详情页只返回 title 不返回正文 → 以 `articles_coverage=stub` 入库；
agent 输出时显式提示 "已知存在但本期未补到正文"，不得把它当作全文依据。
...
export CHINALAW_FETCH_THROTTLE_MS=2000  # 每次 source lookup 至少使用 2 秒间隔
chinalaw fetch ...
```
...
状态: ok / ambiguous / unfindable / fetch_unavailable
覆盖: populated / seed / stub
...
| `chinalaw rebuild-clean --law <name>` | 已入库 → 重清洗（不应替代 fetch） |
