...
Checks chinalaw local database health:
  ...
  - oldest_source_checked_at / oldest_freshness_days within 90 days (warning only)
...
# The status command exposes one conservative oldest-source summary rather than
# a made-up per-source map.  Warn only; freshness does not invalidate the DB.
oldest_checked = payload.get("oldest_source_checked_at")
oldest_days = payload.get("oldest_freshness_days")
if laws_count > 0 and not oldest_checked:
    warnings.append("oldest_source_checked_at is missing")
try:
    if oldest_days is not None and int(oldest_days) > 90:
        warnings.append(
            f"oldest source checked {int(oldest_days)}d ago (>90d threshold)"
        )
except (TypeError, ValueError):
    warnings.append(f"oldest_freshness_days is not an integer: {oldest_days}")
...
if not quiet:
    ...
    print(f"  oldest_checked_at  : {oldest_checked}")
    print(f"  oldest_freshness_d : {oldest_days}")
    print()
