...
# 再按 primary_law_id 取条
chinalaw article 公司法（2023 修订） 第32条 --format json
# 或按已确定的 short_title
...
经典 fly weight：先 `outline 民法典 --part 合同编 第十一章 --full-text` 一次拿
"债权转让"全章节，再 `cited-by 民法典:546` 找司法解释中的关联展开。
...
# 找配套：民法典总则编司法解释
chinalaw search 公序良俗 --kind article --in-part "总则编" --format json
...
对总则编 / 物权编 / 合同编 / 婚姻家庭编 / 继承编 / 侵权责任编都生效。
