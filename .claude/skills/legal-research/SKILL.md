...
司法解释 / 长法会产生很大的 JSON，OpenCode / Claude Code 可能截断工具输出。
...
- `FetchAmbiguousError`：列出候选，让用户或上游规则选择 `--prefer-bbbs`。
...
## Headless 模式（CI / Docker eval / 自动化跑批）

环境变量 `CHINALAW_HEADLESS=1` 或 prompt 含 `--headless` 标记时：

- 跳过"启动协议"里所有交互询问（projectRoot / 案件名 / yes-no 确认）
- 案件信息从 prompt 首行 `[case=<name> question=<text> domain=<area>]` 解析
- **不要写 report.md 到磁盘**：harness 已经用 `claude --print
  --output-format stream-json` 收割了你的最终回答；再写文件等于做两遍工
  作 + 多消耗 turn / output token。直接把法律意见作为最终 assistant
  message 输出即可。
- 仍按"输出要求"写明法规名 + 条号 + 状态 + source_url + 不确定性
- 缺源 / 缺条文 / 时间效力不明时显式报"无法确定"+ 检索线索，不得编造
