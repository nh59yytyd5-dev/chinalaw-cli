...
- 定期保鲜（每周 / 每月增量同步）
- 多项目布局（机密项目要不要独立 db？）
- `chinalaw status` 显示异常（laws 数 / articles 数 / source_freshness）
- 想备份 / 恢复 / 迁移到其他机器
...
chinalaw <cmd>   # 暂未实装环境变量；当前必须显式 --db
```
...
```bash
# 重置：直接删 db 文件，重新跑 sync
rm ~/.chinalaw/chinalaw.db
chinalaw sync --fixtures
chinalaw sync --applicability
...
注意 schema 版本：迁移后跑 `chinalaw status` 会自动 migrate（DDL 在
`src/chinalaw/schema.py`）。如果跨大版本（schema v6 → v8），先备份再迁移。
...
| 字段 | 健康值 | 异常处理 |
|------|-------|---------|
| `laws` | ≥ 仓库 fixture 数（实测 100+） | 远低于 → 重跑 `sync --fixtures` |
...
| `fts_status` | enabled | disabled → 重新 init schema（删 db 重建） |
| `source_freshness` | 各 source 最近 sync 时间 | 远超 30 天 → 重跑 sync |
